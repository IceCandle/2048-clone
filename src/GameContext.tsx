...
const addRandomTile = (board: number[][]): number[][] => {
  ...
  const randomIndex = Math.floor(Math.random() * emptyTiles.length);
	...
	return board.map((row, i) =>
			row.map((val, j) =>
				(i === randomRow && j === randomCol ? newValue : val))
	);
};
...
const moveBoard = (board: number[][], direction: 'UP' | 'DOWN' | 'LEFT' | 'RIGHT'): [number[][], number] => {
  ...
  const rotate = (b: number[][]): number[][] => {
    return (b[0]?.map((_, index) =>
			b.map(row => row[index]).reverse()
		) ?? []) as number[][];
	};
  ...
  if (direction === 'RIGHT') newBoard = rotate(newBoard);
  if (direction === 'DOWN') newBoard = rotate(rotate(newBoard));

  newBoard = moveLeft(newBoard);

  if (direction === 'UP') newBoard = rotate(newBoard);
  if (direction === 'RIGHT') newBoard = rotate(rotate(rotate(newBoard)));
  if (direction === 'DOWN') newBoard = rotate(rotate(newBoard));
...
const gameReducer = (state: GameState, action: GameAction): GameState => {
  switch (action.type) {
    case 'MOVE': {
      ...
      const [newBoard, scoreIncrease] = moveBoard(state.board, action.direction);
      const boardChanged = JSON.stringify(newBoard) !== JSON.stringify(state.board);
      const updatedBoard = boardChanged ? addRandomTile(newBoard) : newBoard;
      const newScore = state.score + scoreIncrease;
      const gameOver = isGameOver(updatedBoard);
      return { board: updatedBoard, score: newScore, gameOver, previousStates: [previousState, ...state.previousStates.slice(0, 9)]};
    }
    case 'NEW_GAME':
      return initialState;
		case 'UNDO': {
